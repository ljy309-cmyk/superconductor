...
class RankingApp(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self,
            text="Qubit Survival Time Leaderboard",
            font=("Consolas", 10),
            bg=BG,
            fg=FG,
        ).pack(pady=(0, 4))
        ...
        tk.Label(row1, text="Player Name:", font=("Consolas", 10), bg=BG, fg=FG, width=14, anchor="w").pack(
            side=tk.LEFT
        )
        ...
        tk.Label(row2, text="Survival (sec):", font=("Consolas", 10), bg=BG, fg=FG, width=14, anchor="w").pack(
            side=tk.LEFT
        )
        ...
        ttk.Combobox(row3, textvariable=self.mode_var, values=modes, width=18, state="readonly").pack(
            side=tk.LEFT, padx=4
        )
        ...
        tk.Label(
            btn_row,
            textvariable=self.submit_status,
            font=("Consolas", 9),
            bg=BG,
            fg=ACCENT,
        ...
        tk.Button(
            tool_row,
            text="Achievements",
            command=self._open_achievements,
            font=("Consolas", 9),
            width=14,
            fg="#f9e2af",
        ).pack(side=tk.LEFT, padx=4)
    ...
    def _load_profile(self):
        ...
        tk.Label(win, text="Select Profile:", font=("Consolas", 10, "bold"), bg=BG, fg=FG).pack(pady=8)
        ...
        tk.Button(win, text="Load", command=_apply, font=("Consolas", 10, "bold"), width=10).pack(pady=8)
    ...
    def _open_achievements(self):
        ...
        try:
            ...
            tk.Label(win, text=f"Achievements: {unlocked}/{total}", font=("Consolas", 14, "bold"), bg=BG, fg=GOLD).pack(
                pady=8
            )

            text = tk.Text(win, bg="#181825", fg=FG, font=("Consolas", 10), state="normal", wrap="word", padx=8, pady=8)
            text.pack(fill=tk.BOTH, expand=True, padx=8, pady=4)

            for ach in achievements:
                status = "[*]" if ach["unlocked"] else "[ ]"
                self._tc["green"] if ach["unlocked"] else get_tk_theme().SUBTEXT
    ...
    def _compare_score(self, my_score: float):
        ...
        if self.top1_score > 0 and my_score > self.top1_score:
            # 내 점수가 1등보다 높으면 → 신기록!
            self.compare_var.set(f"*** 신기록 달성!! *** ({my_score:.2f}s > 기존 1등 {self.top1_score:.2f}s)")
        elif self.top1_score > 0:
            # 낮으면 → 아쉽습니다
            diff = self.top1_score - my_score
            self.compare_var.set(f"아쉽습니다. 다음 기회에! (1등까지 {diff:.2f}초 부족)")
        else:
            # 첫 기록
