...
_HELP_TEXTS: dict[str, list[str]] = {
    ...
    "shor_algorithm": [
        ...
        "",
        "[개요]",
        "  Shor's Algorithm은 양자 컴퓨터로 큰 수를 소인수분해하는",
        "  다항 시간 알고리즘입니다. RSA 암호를 깰 수 있어",
        "  양자 키 분배(QKD)가 필요한 핵심 이유입니다.",
        "",
        "[알고리즘 단계]",
        "  1. 고전 전처리: 짝수/소수/GCD 체크",
        "  2. 랜덤 a 선택: 1 < a < N, gcd(a,N)=1",
        "  3. 양자 주기 탐색: a^x mod N의 주기 r 찾기",
        "     - Hadamard → 모듈러 지수 U_f → QFT† → 측정",
        "  4. 연분수 전개: m/2^n ≈ s/r → r 추출",
        "  5. 인수 추출: gcd(a^(r/2) ± 1, N)",
        "",
        "[핵심 수학]",
        "  ◆ 주기 탐색: a^r ≡ 1 (mod N)인 최소 r",
        "  ◆ QFT: 측정값 m ≈ s·Q/r (Q=2^n)",
        "  ◆ 연분수: m/Q의 수렴분수 분모 → r 후보",
        "  ◆ 인수: N = gcd(a^(r/2)+1, N) × gcd(a^(r/2)-1, N)",
        "",
        "[RSA 위협]",
        "  RSA: N=p×q 소인수분해가 어려움에 의존",
        "  Shor: O((log N)^3) 시간에 소인수분해 가능",
        "  → 양자 컴퓨터가 RSA를 깰 수 있음",
