...
_HELP_TEXTS: dict[str, list[str]] = {
    ...
    "shor_algorithm": [
        ...
        "",
        "[개요]",
        "  Shor's Algorithm(1994, Peter Shor)은 양자 컴퓨터로",
        "  큰 수를 소인수분해하는 다항 시간 알고리즘입니다.",
        "  고전 최선 O(exp(n^{1/3})) → 양자 O((log N)^3)",
        "  RSA 암호를 깰 수 있어 QKD가 필요한 핵심 이유입니다.",
        "",
        "[Step 1. 고전 전처리 (Classical Precheck)]",
        "  양자 회로를 돌리기 전에 고전적으로 걸러냅니다:",
        "  • 짝수 → 2로 나누면 끝 (양자 불필요)",
        "  • 소수 → 소인수분해 자체가 불가",
        "  • 소수 거듭제곱(p^k) → 수치적으로 탐지 가능",
        "  • gcd(a, N) > 1 → 우연히 공통인수 발견!",
        "",
        "[Step 2. 랜덤 a 선택]",
        "  1 < a < N 이고 gcd(a, N) = 1 인 a를 무작위 선택.",
        "  gcd(a, N) > 1 이면 이미 인수를 찾은 것!",
        "  확률적으로 양자 단계 없이 끝날 수도 있습니다.",
        "",
        "[Step 3. 모듈러 지수 계산 (Modular Exponentiation)]",
        "  a^x mod N 을 x=0,1,2,... 로 계산합니다.",
        "  예: a=2, N=15 → 1, 2, 4, 8, 1, 2, 4, 8, ...",
        "  주기 r=4가 반복 패턴으로 보입니다.",
        "  양자 회로에서는 U_f|x⟩|y⟩ = |x⟩|y ⊕ a^x mod N⟩",
        "",
        "[Step 4. 양자 푸리에 변환 (QFT)]",
        "  양자 주기 탐색의 핵심! Hadamard로 중첩 → U_f 적용 후",
        "  QFT†(역푸리에 변환)을 첫 레지스터에 적용합니다.",
        "  측정값 m ≈ s × Q/r (Q=2^n, s는 정수)",
        "  피크가 Q/r 간격으로 나타나는 간섭 패턴!",
        "  수식: P(m) ∝ |Σ exp(2πi·j·m/Q)|^2",
        "",
        "[Step 5. 연분수 전개 (Continued Fractions)]",
        "  측정값 m/Q 를 연분수로 전개합니다:",
        "  m/Q = [a₀; a₁, a₂, ...] → 수렴분수 p_k/q_k",
        "  수렴분수의 분모 q_k 가 주기 r의 후보입니다.",
        "  검증: a^q_k ≡ 1 (mod N) 이면 r = q_k!",
        "  예: 64/256 = 1/4 → r=4 후보",
        "",
        "[Step 6. 인수 추출 (Factor Extraction)]",
        "  주기 r을 찾았으면:",
        "  • r이 짝수인지 확인 (홀수면 실패 → 재시도)",
        "  • a^(r/2) ≡ -1 (mod N)이면 실패 → 재시도",
        "  • p = gcd(a^(r/2) + 1, N)",
        "  • q = gcd(a^(r/2) - 1, N)",
        "  • 1 < p < N 이면 성공! N = p × q",
        "",
        "[성공 확률]",
        "  한 번의 시도로 성공할 확률 ≥ 1/2.",
        "  실패해도 새 a로 재시도하면 O(log log N)번 안에",
        "  거의 확실하게 인수를 찾습니다.",
        "",
        "[RSA 위협 데모]",
        "  RSA: N=p×q 소인수분해의 어려움에 의존",
        "  공개키 (N, e) → 비밀키 d = e^{-1} mod φ(N)",
        "  Shor로 N 분해 → φ(N)=(p-1)(q-1) → d 복원 → 복호화!",
        "  → 양자 컴퓨터가 RSA를 깰 수 있음",
        "  → QKD는 물리 법칙에 기반하여 양자 공격에도 안전",
        "",
        "[조작법]",
        "  Tab: 모드 전환 (Step / Auto / RSA Threat)",
        "  SPACE: 다음 단계 / 시작·정지 / RSA 크래킹",
        "  N: 새 숫자 입력  |  R: 초기화",
        "  ↑/↓: 자동 속도 / RSA 난이도 조절",
    ],
    "grover_search": [
        ...
        "",
        "[개요]",
        "  Grover's Algorithm(1996, Lov Grover)은 비정형 탐색을",
        "  2차(quadratic) 속도 향상으로 수행하는 양자 알고리즘입니다.",
        "  고전 O(N) → 양자 O(√N) — 이것이 증명 가능한 최적입니다.",
        "",
        "[Step 1. 균등 중첩 (Superposition)]",
        "  H⊗n|0⟩ → |ψ⟩ = (1/√N) Σ|x⟩",
        "  모든 N개 상태에 동일한 확률 1/N을 부여합니다.",
        "  이것이 양자 병렬성의 시작입니다.",
        "",
        "[Step 2. Oracle (오라클)]",
        "  마킹 대상의 위상을 반전합니다: |x⟩ → -|x⟩",
        "  확률 자체는 변하지 않지만 부호(위상)가 바뀝니다.",
        "  이것이 양자 '블랙 박스' 쿼리에 해당합니다.",
        "",
        "[Step 3. Diffusion (확산)]",
        "  D = 2|ψ⟩⟨ψ| - I (평균 반전 연산자)",
        "  각 진폭을 평균값 기준으로 반사합니다:",
        "  a_i → 2·mean - a_i",
        "  Oracle이 낮춘 비대상 진폭은 더 낮아지고,",
        "  마킹 상태의 진폭은 증폭됩니다!",
        "",
        "[반복 횟수]",
        "  최적 반복: k = ⌊π/4 × √(N/M)⌋ (M=대상 수)",
        "  반복할수록 대상 확률 ↑ → 최적점에서 ~1",
        "  주의: 최적을 넘기면 확률이 다시 감소합니다!",
        "  수식: P(target) = sin²((2k+1)·θ), θ=arcsin(√(M/N))",
        "",
        "[양자 우위]",
        "  N=1024: 고전 ~1024회 vs Grover ~25회 (40배 빠름)",
        "  N=1,000,000: 고전 ~10^6 vs Grover ~785회",
        "  이 속도 향상은 증명 가능한 최적 — 더 빠른 건 불가능!",
        "",
        "[조작법]",
        "  Tab: 모드 전환 (Step / Auto / Compare)",
        "  SPACE: 다음 단계 / 시작·정지 / 레이스 시작",
        "  N: 새 탐색 설정  |  R: 초기화",
        "  ↑/↓: 자동 속도 / DB 크기 조절",
    ],
    "entanglement": [
