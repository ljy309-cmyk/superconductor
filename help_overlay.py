...
_HELP_TEXTS: dict[str, list[str]] = {
    ...
    "qkd_advanced": [
        ...
        "",
        "[키 시프팅 & 프라이버시 증폭]",
        "  1. 원시 키: Alice/Bob 측정 결과 전체",
        "  2. 키 시프팅: 기저 일치 + 비트 일치만 추출",
        "  3. 프라이버시 증폭: SHA-256 해시로 압축",
        "     → Eve의 부분 정보를 제거하여 보안 키 생성",
        "",
        "[다자간 QKD (GHZ)]",
        "  |GHZ⟩ = (|000⟩+|111⟩)/√2 (3자간 얽힘)",
        "  Z 기저: 모든 파티 같은 결과 → 키 비트",
        "  X 기저: 짝수 패리티 확인 → 일관성 검증",
        "  Eve 도청 시 패리티 깨짐 → 도청 탐지",
        "",
        "[조작법]",
        "  Tab: 모드 전환 (E91/Sift/GHZ)",
        "  SPACE: 배치 실행  |  S: 키 시프팅/PA",
        "  E: Eve 도청 ON/OFF  |  A: 자동 실행",
        "  P: 일시정지  |  R: 리셋",
    ],
    "squid_mines": [
