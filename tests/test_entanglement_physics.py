...
class TestBellStates(unittest.TestCase):
    ...
    def test_four_bell_states_exist(self):
        from quantum.entanglement_physics import BELL_LABELS, BELL_STATES
    ...
    def test_bell_states_normalized(self):
        ...
        for name, state in BELL_STATES.items():
            ...
            self.assertAlmostEqual(norm_sq, 1.0, places=10, msg=f"|{name}⟩ not normalized: {norm_sq}")
...
class TestBellMeasurement(unittest.TestCase):
    ...
    def test_measurement_returns_binary(self):
        ...
        for _name, state in BELL_STATES.items():
            a, b = measure_bell(state)
            self.assertIn(a, (0, 1))
            self.assertIn(b, (0, 1))
...
class TestCHSH(unittest.TestCase):
    ...
    def test_chsh_violation_with_bell_state(self):
        ...
        for _ in range(n_trials):
            ...
            if result["violated"]:
                violations += 1
        self.assertGreater(violations, n_trials // 2, f"Only {violations}/{n_trials} violations")
...
class TestTeleportation(unittest.TestCase):
    ...
    def test_teleportation_full_protocol(self):
        ...
        self.assertAlmostEqual(ts.fidelity, 1.0, places=6, msg="Teleportation fidelity should be ~1.0")
    ...
    def test_teleportation_multiple_random_states(self):
        ...
        for _ in range(20):
            ...
            for _ in range(6):
                teleport_step(ts)
            self.assertAlmostEqual(ts.fidelity, 1.0, places=5, msg=f"Fidelity {ts.fidelity} for α={alpha}, β={beta}")
