...
class TestRankingValidation(unittest.TestCase):
    ...
    def test_score_clamping(self):
        ...
        self.assertEqual(max(0.0, min(float(42.5), 999999.0)), 42.5)
...
class TestQubitNetwork(unittest.TestCase):
    ...
    def test_update_causes_collapse(self):
        ...
        collapsed = net.update(0.1, noise_rate=50.0, cascade_damage=20.0,
                               shield_active=False, qec_reduction=0.2)
        # 높은 노이즈로 최소 1개는 붕괴
    ...
    def test_shield_reduces_noise(self):
        ...
        import random
        random.seed(42)
        net1.update(1.0, noise_rate=10.0, cascade_damage=20.0,
                    shield_active=False, qec_reduction=0.2)
        stress_no_shield = sum(n.stress for n in net1.nodes)

        random.seed(42)
        net2.update(1.0, noise_rate=10.0, cascade_damage=20.0,
                    shield_active=True, qec_reduction=0.2)
