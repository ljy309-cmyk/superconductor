...
class QubitPacket:
    ...
    def __init__(self, bit: str, basis: str, round_id: int):
        self.bit = bit
        self.basis = basis
...
class BB84Game:
    ...
    def new_round(self):
        ...
        pkt = QubitPacket(alice_bit, alice_basis, self.round_id)
    ...
    def process_arrival(self, pkt: QubitPacket):
        ...
        if basis_match:
            ...
            if len(self.error_history) > HISTORY_WINDOW:
        ...
        # 자동 통신망 폐쇄
        if self.error_rate >= ERROR_THRESHOLD and len(self.error_history) >= 5:
...
def _draw_packets(screen, game: BB84Game, font):
    ...
    for pkt in game.packets:
        ...
        color = EVE_CLR if pkt.corrupted else QUBIT_CLR
        pygame.draw.circle(screen, color, (cx, cy), 12)
        pygame.draw.circle(screen, TEXT_CLR, (cx, cy), 12, 1)
        sym = font.render(pkt.display, True, (255, 255, 255))
...
def _draw_error_meter(screen, game: BB84Game, font, big_font):
    ...
    label = big_font.render("Error Rate", True, ACCENT)
    ...
    pygame.draw.line(screen, SHUTDOWN_CLR, (thresh_x, my - 4), (thresh_x, my + mh + 4), 1)
    thresh_label = font.render(f"{ERROR_THRESHOLD * 100:.0f}%", True, SHUTDOWN_CLR)
    screen.blit(thresh_label, (thresh_x - thresh_label.get_width() // 2, my + mh + 6))

    # 채움
    fill_w = int(mw * min(game.error_rate, 1.0))
    fill_clr = DANGER_CLR if game.error_rate >= ERROR_THRESHOLD else SAFE_CLR
...
def _draw_stats(screen, game: BB84Game, font):
    """통계 패널."""
    sx, sy = 50, 440
    lines = [
        ...
        (f"채널: {'OPEN' if game.channel_open else 'SHUTDOWN'}", SAFE_CLR if game.channel_open else DANGER_CLR),
    ]
...
def _draw_log(screen, game: BB84Game, font):
    ...
    for i, entry in enumerate(game.log):
        ...
        color = DANGER_CLR if entry["error"] else EVE_CLR if entry["intercepted"] else TEXT_CLR
        text = f"R{r:03d} A:{ab} B:{bb} {match}{eve}{err}"
        surf = font.render(text, True, color)
...
def run_simulation():
    ...
    while running:
        ...
        # 안내
        hints = [
            "SPACE: 통신망 폐쇄/재개  |  P: 일시정지  |  R: 리셋  |  ESC: 종료",
        ]
